...
class PathInterpolatorApi14 implements Interpolator {
    ...
    /* renamed from: mX */
    private final float[] f5mX;

    /* renamed from: mY */
    private final float[] f6mY;

    PathInterpolatorApi14(Path path) {
        PathMeasure pathMeasure = new PathMeasure(path, false);
        float pathLength = pathMeasure.getLength();
        int numPoints = ((int) (pathLength / PRECISION)) + 1;
        this.f5mX = new float[numPoints];
        this.f6mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, null);
            this.f5mX[i] = position[0];
            this.f6mY[i] = position[1];
        }
    }
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f5mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f5mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        }
        float xRange = this.f5mX[endIndex] - this.f5mX[startIndex];
        if (xRange == 0.0f) {
            return this.f6mY[startIndex];
        }
        float fraction = (t - this.f5mX[startIndex]) / xRange;
        float startY = this.f6mY[startIndex];
        return ((this.f6mY[endIndex] - startY) * fraction) + startY;
    }
