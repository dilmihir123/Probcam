...
class FragmentTransition {
    ...
    public static void calculateFragments(BackStackRecord transaction, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isReordered) {
        ...
        for (int opNum = 0; opNum < numOps; opNum++) {
            addToFirstInLastOut(transaction, (C0025Op) transaction.mOps.get(opNum), transitioningFragments, false, isReordered);
        }
    }
    ...
    public static void calculatePopFragments(BackStackRecord transaction, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isReordered) {
        if (transaction.mManager.mContainer.onHasView()) {
            for (int opNum = transaction.mOps.size() - 1; opNum >= 0; opNum--) {
                addToFirstInLastOut(transaction, (C0025Op) transaction.mOps.get(opNum), transitioningFragments, true, isReordered);
            }
        }
    }
    ...
    private static void addToFirstInLastOut(BackStackRecord transaction, C0025Op op, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isPop, boolean isReorderedTransaction) {
